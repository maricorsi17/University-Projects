...
plot(as.zoo(tsNH4OUTtotal),type="n", xlab="Mesi",ylab=expression(paste("N-NH"[4-OUT]^"+"," [mg/L]")),yaxt="n",xaxt="n",yaxs="i",xaxs="i",cex.lab=1.2,ylim=c(0,15),col="orange")
...
text(x=index(tsNtotIN2015[182,]),y=14,label="2015")
text(x=index(tsNtotIN2016[182,]),y=14,label="2016")
text(x=index(tsNtotIN2017[182,]),y=14,label="2017")
text(x=index(tsNtotIN2018[90,]),y=14,label="2018")
text(x=index(tsNtotIN2016[181,]),y=7.5, label=paste("Variazione = ",format(round(perc_a,digits=1),decimal.mark = ","),"%"),col="red")
legend(x=index(tsNtotIN2017[220,]),y=13, c(expression(paste("N-NH"[4-OUT]^"+")), "Regressione","LOESS"),col=c("darkslategrey","red","black"),lty=c(1,5,1),lwd=c(1,2,2),bg="white")
...
plot(as.zoo(tsNH4OUTtotal),type="n", xlab="Mesi",ylab=expression(paste("N-NO"[2-OUT]^"-"," [mg/L]")),yaxt="n",xaxt="n",yaxs="i",xaxs="i",cex.lab=1.2,ylim=c(0,1),col="orange")
drawTimeAxis(as.zoo(tsNH4OUTtotal_original), tick.tstep ="months", lab.tstep ="months",las=2,lab.fmt="%m")
...
text(x=index(tsNtotIN2016[181,]),y=0.5, label=paste("Variazione = ",format(round(perc_a1,digits=1),decimal.mark = ","),"%"),col="red")
legend(x=index(tsNtotIN2017[220,]),y=0.85, c(expression(paste("N-NO"[2-OUT]^"-")),"Regressione","LOESS"),col=c("darkslategrey","red","black"),lty=c(1,5,1),lwd=c(1,2,2),bg="white")
...
text(x=index(tsNtotIN2016[181,]),y=14, label=paste("Variazione = ",format(round(perc_a2,digits=1),decimal.mark = ","),"%"),col="red")
legend(x=index(tsNtotIN2017[220,]),y=21, c(expression(paste("N-NO"[3-OUT]^"-")),"Regressione","LOESS"),col=c("darkslategrey","red","black"),lty=c(1,5,1),lwd=c(1,2,2),bg="white")
