...
plot(as.zoo(tsBODINtotal),type="n", xlab="Mesi",ylab=expression(paste("BOD"[5-OUT]," [mg/L]")),yaxt="n",xaxt="n",yaxs="i",xaxs="i",cex.lab=1.2,ylim=c(0,50),col="grey")
...
text(x=index(tsBODIN2015[182,]),y=47.5,label="2015")
text(x=index(tsBODIN2016[182,]),y=47.5,label="2016")
text(x=index(tsBODIN2017[182,]),y=47.5,label="2017")
text(x=index(tsBODIN2018[90,]),y=47.5,label="2018")
text(x=index(tsBODIN2016[181,]),y=22.5, label=paste("Variazione = ",format(round(perc_a,digits=1),decimal.mark = ","),"%"),col="red")
legend(x=index(tsBODIN2015[30,]),y=42.0, c(expression(paste("BOD"[5-OUT])),"Regressione","LOESS"),col=c("darkslategrey","red","black"),lty=c(1,5,1),lwd=c(1,2,2),bg="white")
