...
plot(as.zoo(tsNO3INtotal),type="n", xlab="Mesi",ylab=expression(paste("N-NO"[3-IN]^"-"," [mg/L]")),yaxt="n",xaxt="n",yaxs="i",xaxs="i",cex.lab=1.2,ylim=c(0,2),lwd=1,col="grey")
...
text(x=index(tsNtotIN2016[195,]),y=0.2, label=paste("Variazione = ",format(round(perc_a,digits=1),decimal.mark = ","),"%"),col="red")
legend(x=index(tsNtotIN2015[30,]),y=1.8, c(expression(paste("N-NO"[3-IN]^"-")),"Regressione","LOESS"),col=c("darkslategrey","red","black"),lty=c(1,5,1),lwd=c(1,2,2),bg="white")
...
plot(as.zoo(tsNO2INtotal),type="n", xlab="Mesi",ylab=expression(paste("N-NO"[2-IN]^"-"," [mg/L]")),yaxt="n",xaxt="n",yaxs="i",xaxs="i",cex.lab=1.2,ylim=c(0,0.75),lwd=1,col="grey")
drawTimeAxis(as.zoo(tsNtotINtotal_original), tick.tstep ="months", lab.tstep ="months",las=2,lab.fmt="%m")
...
abline(v=index(tsNtotIN2017[1,]),lwd=2)
abline(v=index(tsNtotIN2018[1,]),lwd=2)
text(x=index(tsNtotIN2015[182,]),y=0.725,label="2015")
text(x=index(tsNtotIN2016[182,]),y=0.725,label="2016")
text(x=index(tsNtotIN2017[182,]),y=0.725,label="2017")
text(x=index(tsNtotIN2018[90,]),y=0.725,label="2018")
text(x=index(tsNtotIN2016[181,]),y=0.3, label=paste("Variazione = ",format(round(perc_a1,digits=1),decimal.mark = ","),"%"),col="red")
legend(x=index(tsNtotIN2015[30,]),y=0.69, c(expression(paste("N-NO"[2-IN]^"-")),"Regressione","LOESS"),col=c("darkslategrey","red","black"),lty=c(1,5,1),lwd=c(1,2,2),bg="white")
