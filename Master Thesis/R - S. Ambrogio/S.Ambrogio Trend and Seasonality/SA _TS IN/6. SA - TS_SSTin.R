...
plot(as.zoo(tsSSTINtotal),type="n",xlab="Mesi",ylab=expression(paste("SST"[IN]," [mg/L]")),yaxt="n",xaxt="n",yaxs="i",xaxs="i",cex.lab=1.2,ylim=c(0,1500),col="grey")
drawTimeAxis(as.zoo(tsSSTINtotal_original), tick.tstep ="months", lab.tstep ="months",las=2,lab.fmt="%m")
axis(side=2,at=seq(from = 0,to = 1500,by = 250),las=2,format(seq(from = 0,to = 1500,by = 250), big.mark = ".", decimal.mark = ","))
...
text(x=index(tsSSTIN2015[182,]),y=1375,label="2015")
text(x=index(tsSSTIN2016[182,]),y=1375,label="2016")
text(x=index(tsSSTIN2017[182,]),y=1375,label="2017")
text(x=index(tsSSTIN2018[90,]),y=1375,label="2018")
text(x=index(tsSSTIN2016[181,]),y=100, label=paste("Variazione = ",format(round(perc_a,digits=1),decimal.mark = ",",nsmall=1),"%"),col="red")
legend(x=index(tsSSTIN2015[30,]),y=1260, c(expression(paste("SST"[IN])),"Regressione","LOESS"),col=c("darkslategrey","red","black"),lty=c(1,5,1),lwd=c(1,2,2),bg="white")
