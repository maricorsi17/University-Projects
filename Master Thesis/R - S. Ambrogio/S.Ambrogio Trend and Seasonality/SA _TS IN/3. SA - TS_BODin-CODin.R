...
plot(as.zoo(tsBODINtotal),type="n", xlab="Mesi",ylab=expression(paste("BOD"[5-IN]," [mg/L]")),yaxt="n",xaxt="n",yaxs="i",xaxs="i",cex.lab=1.2,ylim=c(0,1000),col="grey")
drawTimeAxis(as.zoo(tsBODINtotal_original), tick.tstep ="months", lab.tstep ="months",las=2,lab.fmt="%m")
axis(side=2,at=seq(from = 0,to = 1000,by = 100),las=2,format(seq(from = 0,to = 1000,by = 100), big.mark = ".", decimal.mark = ","))
...
text(x=index(tsBODIN2015[182,]),y=950,label="2015")
text(x=index(tsBODIN2016[182,]),y=950,label="2016")
text(x=index(tsBODIN2017[182,]),y=950,label="2017")
text(x=index(tsBODIN2018[90,]),y=950,label="2018")
text(x=index(tsBODIN2016[181,]),y=650, label=paste("Variazione = ",format(round(perc_a,digits=1),decimal.mark = ","),"%"),col="red")
legend(x=index(tsBODIN2015[30,]),y=850, c(expression(paste("BOD"[5-IN])),"Regressione","LOESS"),col=c("darkslategrey","red","black"),lty=c(1,5,1),lwd=c(1,2,2),bg="white")
...
plot(as.zoo(tsBODINtotal),type="n", xlab="Mesi",ylab=expression(paste("COD"[IN]," [mg/L]")),yaxt="n",xaxt="n",yaxs="i",xaxs="i",cex.lab=1.2,ylim=c(0,4000),col="grey")
drawTimeAxis(as.zoo(tsBODINtotal_original), tick.tstep ="months", lab.tstep ="months",las=2,lab.fmt="%m")
...
text(x=index(tsBODIN2016[181,]),y=1750, label=paste("Variazione = ",format(round(perc_a1,digits=1),decimal.mark = ","),"%"),col="red")
legend(x=index(tsBODIN2015[30,]),y=3450, c(expression(paste("COD"[IN])),"Regressione","LOESS"),col=c("darkslategrey","red","black"),lty=c(1,5,1),lwd=c(1,2,2),bg="white")


# DETRENDING
tsBODINtotalDET<-tsBODINtotal-(ts_trend(tsBODINtotal))
mediaBODDET<-mean(tsBODINtotalDET)

tsCODINtotalDET<-tsCODINtotal-(ts_trend(tsCODINtotal))
mediaCODDET<-mean(tsCODINtotalDET)
