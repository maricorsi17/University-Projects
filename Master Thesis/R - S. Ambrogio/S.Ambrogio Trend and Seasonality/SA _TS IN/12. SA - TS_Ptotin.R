...
text(x=index(tsPtotIN2015[182,]),y=23,label="2015")
text(x=index(tsPtotIN2016[182,]),y=23,label="2016")
text(x=index(tsPtotIN2017[182,]),y=23,label="2017")
text(x=index(tsPtotIN2018[90,]),y=23,label="2018")
text(x=index(tsPtotIN2016[181,]),y=17.5, label=paste("Variazione = ",format(round(perc_a,digits=1),decimal.mark = ","),"%"),col="red")

legend(x=index(tsPtotIN2015[30,]),y=21, c(expression(paste("P"[tot-IN])),"Regressione","LOESS"),col=c("darkslategrey","red","black"),lty=c(1,5,1),lwd=c(1,2,2),bg="white")
