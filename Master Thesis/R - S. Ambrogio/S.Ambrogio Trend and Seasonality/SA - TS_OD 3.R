...
plot(as.zoo(tsOD),type="n", xlab="Mesi",ylab=expression(paste("OD linea 3 [mg/L]")),yaxt="n",xaxt="n",yaxs="i",xaxs="i",cex.lab=1.2,ylim=c(0,12.5))
...
text(x=index(tsOD[43800,]),y=8.75, label=paste("Variazione = ",format(round(perc_a,digits=1),decimal.mark = ",",nsmall=1),"%"),col="red")
legend(x=index(tsOD[10000,]),y=10.75, c(expression(paste("OD")),"Regressione","LOESS"),col=c("darkslategrey","red","black"),lty=c(1,5,1),lwd=c(1,2,2),bg="white")
...
options(OutDec= ",")
acf(coredata(tsODAGG),lag.max = 60, main=expression("OD linea 3"),yaxt="n", ci.col="black",cex.lab=1.2)
axis(side=2,las=2)