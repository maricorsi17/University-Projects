...
tsCaricoBODOUTtotal<-(na.omit(xts(x=SAmbrogio[,18], order.by=datestotal))*xts(x=SAmbrogio[,8],order.by=datestotal)/1000)
...
tsCaricoCODOUTtotal<-(na.omit(xts(x=SAmbrogio[,20], order.by=datestotal))*xts(x=SAmbrogio[,8],order.by=datestotal)/1000)
...
tsCaricoNtotOUTtotal<-(na.omit(xts(x=SAmbrogio[,32], order.by=datestotal))*xts(x=SAmbrogio[,8],order.by=datestotal)/1000)
...
tsCaricoPtotOUTtotal<-(na.omit(xts(x=SAmbrogio[,30], order.by=datestotal))*xts(x=SAmbrogio[,8],order.by=datestotal)/1000)
...
# tsCaricoSSTOUTtotal<-(na.omit(xts(x=SAmbrogio[,16], order.by=datestotal))*xts(x=SAmbrogio[,8],order.by=datestotal)/1000)
...
#NIT totale
tsCaricoTKNINtotal<-(na.omit(xts(x=SAmbrogio[,33], order.by=datestotal))*xts(x=SAmbrogio[,6],order.by=datestotal)/1000)
mediaCaricoTKNINtotal<-mean(tsCaricoTKNINtotal)
tsCaricoNNH4OUTtotal<-(na.omit(xts(x=SAmbrogio[,22], order.by=datestotal))*0.78*xts(x=SAmbrogio[,8],order.by=datestotal)/1000)
...
#DEN totale
tsCaricoNNO3OUTtotal<-(na.omit(xts(x=SAmbrogio[,26], order.by=datestotal))*xts(x=SAmbrogio[,8],order.by=datestotal)/1000)
mediaCaricoNNO3OUTtotal<-mean(tsCaricoNNO3OUTtotal)
tsCaricoNNO2OUTtotal<-(na.omit(xts(x=SAmbrogio[,24], order.by=datestotal))*xts(x=SAmbrogio[,8],order.by=datestotal)/1000)
...
options(OutDec = ",")
bp<-barplot(as.matrix(df),beside=T,ylim=c(0,100),col=brewer.pal(6,"Set3"),yaxt="n",xaxt="n")
...
bp1<-barplot(as.matrix(df1),beside=T,ylim=c(0,55000),col=brewer.pal(4,"Set3"),yaxt="n",xaxt="n")
grid(nx=NA,ny=11,col="grey")
barplot(as.matrix(df1),beside=T,ylim=c(0,55000),col=brewer.pal(4,"Set3"),yaxt="n",ylab=expression(paste("BOD"[5-IN],", COD"[IN],", N "[tot-IN],", P"[tot-IN]," [AE]")),ad=T,las=1)
...
axis(side=2,at=seq(from = 0, to = 55000, by = 5000), las=2,format(seq(from = 0,to = 55000,by = 5000), big.mark = ".", decimal.mark = ","))
abline(h=55000,lty=3,col="grey")
abline(h=0)
text(bp1,as.matrix(df1)/2,as.character(as.matrix(format(round(df1),big.mark=".", decimal.mark = ","))),srt=90)
text(bp1,percentiliBOD+3000,as.character(format(round(percentiliBOD),big.mark=".", decimal.mark = ",")),srt=90)
text(bp1,percentiliCOD+3000,as.character(format(round(percentiliCOD),big.mark=".", decimal.mark = ",")),srt=90)


legend(x=8,y=52000,ncol=3, c(expression("BOD"[5-IN]),expression("COD"[IN]),expression("N"[tot-IN]),expression("P"[tot-IN]),"90° percentile","92° percentile"),pch=c(22,22,22,22,NA,NA),col=c(NA,NA,NA,NA,"black","blue"),lty=c(NA,NA,NA,NA,1,1),pt.bg=brewer.pal(4,"Set3"),pt.cex = 2,bg="white")

windows(width = 16,height = 9)
par(mar=c(6,6,4,6),mgp=c(3,1,0)) #margini e distanza etichette-asse

aetotal<-rev(c(aeBODtotal,aeCODtotal,aeNtottotal,aePtottotal))
bpt<-barplot(aetotal,horiz = T, xlim=c(0,25000),space=c(0,0),xaxt="n",xlab=expression(paste("BOD"[5-IN],", COD"[IN],", N "[tot-IN],", P"[tot-IN]," [AE]")),ylab="2015 - 2018",col=rev(brewer.pal(4,"Set3")))
axis(side=1,at=seq(from = 0, to = 25000, by = 5000), las=1,format(seq(from = 0,to = 25000,by = 5000), big.mark = ".", decimal.mark = ","))
text(aetotal/2,bpt,as.character(format(round(aetotal), big.mark = ".", decimal.mark = ",")))
legend(x=21000,y=3.5, c(expression("BOD"[5-IN]),expression("COD"[IN]),expression("N"[tot-IN]),expression("P"[tot-IN])),pch=c(22,22,22,22),pt.bg=brewer.pal(4,"Set3"),pt.cex = 2,bg="white")
abline(v=0)