...
tsTOUTtotal<-na.approx(xts(x=SAmbrogio[,5], order.by=datestotal))
tsTINtotalNA<-(xts(x=SAmbrogio[,4], order.by=datestotal))
tsTOUTtotalNA<-(xts(x=SAmbrogio[,5], order.by=datestotal))
...
legend(x=index(tsTIN2018[20,]),y= 28, c(expression(paste("T"[IN])),expression(paste("T"[OUT]))),col=c("black","red"),lty=c(1,1),lwd=c(1,1),bg="white")
...
text(x=7.25,y=27.5, label=paste("r = ",format(round(corr$estimate,digits=2),decimal.mark=",")))
