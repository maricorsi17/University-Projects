...
tsSSTOUTtotalNA<-(xts(x=SAmbrogio[,16], order.by=datestotal))
...
plot(as.zoo(tsQOUTtotal), type = "l", yaxt="n",xaxt="n",xaxs="i",yaxs="i", bty="n", xlab="", ylab="", ylim=c(0,8000), col="orange")
lines(as.zoo(MAtsQOUTtotal[index(tsQOUTtotal)]),col="red")
axis(side=4, at=seq(from=0, to=8000, by=1000),las=2,format(seq(from = 0,to = 8000,by = 1000), big.mark = ".", decimal.mark = ","))
...
text(x=index(tsSSTOUT2015[182,]),y=7750,label="2015")
text(x=index(tsSSTOUT2016[182,]),y=7750,label="2016")
text(x=index(tsSSTOUT2017[182,]),y=7750,label="2017")
text(x=index(tsSSTOUT2018[90,]),y=7750,label="2018")
legend(x=index(tsSSTOUT2017[220,]),y=6500, c(expression(paste("SST"[OUT])),expression(paste("Q"[OUT])),expression(paste("MA SST"[OUT]," (mensile)")),expression(paste("MA Q"[OUT]," (mensile)")),"Limite"),col=c("darkslategrey","orange","black","red","blue"),lty=c(1,1,1,1,2),lwd=c(1,1,1,1,1),bg="white")
...
# SCATTERPLOT
corr<-cor.test(coredata(tsSSTOUTtotalNA),coredata(tsQOUTtotal),method = "spearman", use="complete.obs",exact = F)

windows(width = 16,height = 9)
par(mar=c(6,6,4,4),mgp=c(4,1,0))
plot(as.zoo(tsSSTOUTtotal),as.zoo(tsQOUTtotal),xaxs="i",yaxt="n",xaxt="n",yaxs="i",xlab=expression(paste("SST"[OUT]," [mg/L]")),ylab=expression(paste("Q [m"^"3","/d]")),cex.lab="1.2",xlim=c(0,40),ylim=c(0,8000),pch=16)
axis(side=2,at=seq(from = 0,to = 8000,by = 1000),las=2,format(seq(from = 0,to = 8000,by = 1000), big.mark = ".", decimal.mark = ","))
axis(side=1,at=seq(from = 0,to = 40,by = 10),las=1,format(seq(from = 0,to = 40,by = 10), big.mark = ".", decimal.mark = ","))
grid(nx=4,ny=8,col="grey")

abline(lm(tsQOUTtotal~tsSSTOUTtotalNA),lwd=2)

text(x=25,y=7500, label=paste("r = ",format(round(corr$estimate,digits=2),decimal.mark=",")))
