...
tsCODOUTtotalNA<-(xts(x=SAmbrogio[,20], order.by=datestotal))
...
lines(as.zoo(MAtsCODINtotal[index(tsCODINtotal)]))
...
legend(x=index(tsCODIN2015[30,]),y=1800, c(expression(paste("COD"[IN])),expression(paste("COD"[OUT])),expression(paste("MA COD"[IN]," (mensile)")),expression(paste("MA COD"[OUT]," (mensile)")),"Limite"),col=c("darkslategrey","orange","black","red","blue"),lty=c(1,1,1,1,2),lwd=c(1,1,1,1,1),bg="white")

# BOXPLOT
windows(width = 6,height = 6)
par(mar=c(1,6,4,4),mgp=c(4,1,0))

boxplot(coredata(tsCODOUTtotal),yaxt="n",ylab="[mg/L]",main=expression(paste("COD"[OUT])), ylim=c(0,120),cex.lab=1.5,cex.main=2)
...
text(x=250,y=112.5, label=paste("r = ",format(round(corr$estimate,digits=2),nsmall=2,decimal.mark=",")))
