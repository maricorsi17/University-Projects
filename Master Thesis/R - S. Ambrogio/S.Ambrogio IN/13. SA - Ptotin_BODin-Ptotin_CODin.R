...
plot(as.zoo(rapportoP_BOD),type="n",xlab="Mesi",ylab=expression(paste("P"[tot-IN],"/BOD"[5-IN],", P"[tot-IN],"/COD"[IN]," [ - ]")),yaxt="n",xaxt="n",yaxs="i",xaxs="i",cex.lab=1.2,ylim=c(0,0.08),col="grey")
...
legend(x=index(tsPtotIN2015[30,]),y=0.072, c(expression(paste("P"[tot-IN],"/BOD"[5-IN])),expression(paste("MA P"[tot-IN],"/BOD"[5-IN]," (mensile)")),expression(paste("P"[tot-IN],"/COD"[IN])), expression(paste("MA P"[tot-IN],"/COD"[IN], " (mensile)"))),col=c("darkslategrey","black","orange","red"),lty=c(1,1,1,1),lwd=c(1,1,1,1),bg="white")
...
boxplot(coredata(rapportoP_BOD),yaxt="n",ylab="[ - ]",main=expression(paste("P"[tot-IN],"/BOD"[5-IN])), ylim=c(0,0.08),cex.lab=1.5,cex.main=2)
axis(side=2,at=seq(from = 0,to = 0.08,by = 0.02),las=2,labels = format(seq(from = 0,to = 0.08,by = 0.02), big.mark = ".", decimal.mark = ","))

windows(width = 6,height = 6)
par(mar=c(1,6,4,4),mgp=c(4,1,0))

boxplot(coredata(rapportoP_COD),yaxt="n",ylab="[ - ]",main=expression(paste("P"[tot-IN],"/COD"[IN])), ylim=c(0,0.04),cex.lab=1.5,cex.main=2)
axis(side=2,at=seq(from = 0,to = 0.04,by = 0.01),las=2,labels = format(seq(from = 0,to = 0.04,by = 0.01), big.mark = ".", decimal.mark = ","))
