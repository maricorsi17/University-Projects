...
tsNtotOUTtotal_original<-na.approx(xts(x=Caprino[,30], order.by=datestotal))
tsNtotOUTtotal<-na.omit(xts(x=Caprino[,30], order.by=datestotal))
MAtsNtotOUTtotal<-na.omit(rollapply(xts(x=Caprino[,30], order.by=datestotal), width=31, FUN=function(x) mean(x, na.rm=TRUE), by=1, by.column=TRUE, fill=NA, align="center"))
tsNtotOUTtotalNA<-(xts(x=Caprino[,30], order.by=datestotal))



## Creare un oggetto con le date (2015)
tsNtotIN2015<-tsNtotOUTtotal_original["2015"]

## Creare un oggetto con le date (2016)
tsNtotIN2016<-tsNtotOUTtotal_original["2016"]

## Creare un oggetto con le date (2017)
tsNtotIN2017<-tsNtotOUTtotal_original["2017"]

## Creare un oggetto con le date (2018)
tsNtotIN2018<-tsNtotOUTtotal_original["2018"]
...
text(x=index(tsNtotIN2015[182,]),y=140,label="2015")
text(x=index(tsNtotIN2016[182,]),y=140,label="2016")
text(x=index(tsNtotIN2017[182,]),y=140,label="2017")
text(x=index(tsNtotIN2018[90,]),y=140,label="2018")
legend(x=index(tsNtotIN2017[180,]),y=130, c(expression(paste("N"[tot-IN])), expression(paste("N"[tot-OUT])),expression(paste("MA N"[tot-IN]," (mensile)")),expression(paste("MA N"[tot-OUT]," (mensile)")),"Limite"),col=c("darkslategrey","orange","black","red","blue"),lty=c(1,1,1,1,2),lwd=c(1,1,1,1,1),bg="white")

# BOXPLOT
windows(width = 6,height = 6)
par(mar=c(1,6,4,4),mgp=c(4,1,0))

boxplot(coredata(tsNtotOUTtotal),yaxt="n",ylab="[mg/L]",main=expression(paste("N"[tot-OUT])), ylim=c(0,18),cex.lab=1.5,cex.main=2)
axis(side=2,at=seq(from = 0,to = 18,by = 2),las=2,labels = format(seq(from = 0,to = 18,by = 2), big.mark = ".", decimal.mark = ","))
