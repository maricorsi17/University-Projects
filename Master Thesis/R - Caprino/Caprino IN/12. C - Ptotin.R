...
legend(x=index(tsPtotIN2017[220,]),y=22, c(expression(paste("P"[tot-IN])),expression(paste("MA P"[tot-IN]," (mensile)"))),col=c("darkslategrey","black"),lty=c(1,1),lwd=c(1,1),bg="white")

# BOXPLOT
windows(width = 6,height = 6)
par(mar=c(1,6,4,4),mgp=c(4,1,0))

boxplot(coredata(tsPtotINtotal),yaxt="n",ylab="[mg/L]",main=expression(paste("P"[tot-IN])), ylim=c(0,20),cex.lab=1.5,cex.main=2)
