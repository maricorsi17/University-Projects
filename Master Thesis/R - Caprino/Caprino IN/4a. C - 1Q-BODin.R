...
MAtsBODINtotal<-na.omit(rollapply(xts(x=Caprino[,15], order.by=datestotal), width=15, FUN=function(x) mean(x, na.rm=TRUE), by=1, by.column=TRUE, fill=NA, align="center"))
...
plot(as.zoo(tsBODINtotal), type = "l", yaxt="n",xaxt="n",xaxs="i",yaxs="i", bty="n", xlab="", ylab="", ylim=c(0,2400), col="orange")
lines(as.zoo(MAtsBODINtotal[index(tsBODINtotal)]),col="red")
axis(side=4, at=seq(from=0, to=2400, by=400),las=2,format(seq(from = 0,to = 2400,by = 400), big.mark = ".", decimal.mark = ","))
...
text(x=index(tsQOUT2015[182,]),y=2300,label="2015")
text(x=index(tsQOUT2016[182,]),y=2300,label="2016")
text(x=index(tsQOUT2017[182,]),y=2300,label="2017")
text(x=index(tsQOUT2018[90,]),y=2300,label="2018")
legend(x=index(tsQOUT2017[160,]),y=2100, c("1/Q",expression(paste("BOD"[5-IN])),"MA 1/Q (bisettimanale)",expression(paste("MA BOD"[5-IN]," (bisettimanale)"))),col=c("darkslategrey","orange","black","red"),lty=c(1,1,1,1),lwd=c(1,1,1,1),bg="white")
...
plot(as.zoo(tsBODINtotal),as.zoo(ts1QOUTtotal),xaxs="i",yaxt="n",xaxt="n",yaxs="i",xlab=expression(paste("BOD"[5-IN]," [mg/L]")),ylab=expression(paste("1/Q [d/m"^"3","]")),cex.lab="1.2",xlim=c(0,2400),ylim=c(0,0.003),pch=16)
axis(side=2,at=seq(from = 0,to = 0.003,by = 0.0005),las=2,format(seq(from = 0,to = 0.003,by = 0.0005), big.mark = ".", decimal.mark = ","),pch=16)
axis(side=1,at=seq(from = 0,to = 2400,by = 400),las=1,format(seq(from = 0,to = 2400,by = 400), big.mark = ".", decimal.mark = ","))
grid(nx=6,ny=6,col="grey")

abline(lm(ts1QOUTtotal~tsBODINtotalNA),lwd=2)

text(x=200,y=0.00275, label=paste("r = ",format(round(corr$estimate,digits=2),decimal.mark=",")))
