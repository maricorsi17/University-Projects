...
plot(as.zoo(tsSSTINtotal),type="n", xlab="Mesi",ylab=expression(paste("SST"[IN],", COD"[IN]," [mg/L]")),yaxt="n",xaxt="n",yaxs="i",xaxs="i",cex.lab=1.2,ylim=c(0,5000),col="grey")
...
legend(x=index(tsSSTIN2017[220,]),y=4500, c(expression(paste("SST"[IN])),expression(paste("COD"[IN])),expression(paste("MA SST"[IN]," (mensile)")),expression(paste("MA COD"[IN]," (mensile)"))),col=c("darkslategrey","orange","black","red"),lty=c(1,1,1,1),lwd=c(1,1,1,1),bg="white")

# BOXPLOT
windows(width = 6,height = 6)
par(mar=c(1,6,4,4),mgp=c(4,1,0))

boxplot(coredata(tsSSTINtotal),yaxt="n",ylab="[mg/L]",main=expression(paste("SST"[IN])), ylim=c(0,5000),cex.lab=1.5,cex.main=2)
axis(side=2,at=seq(from = 0,to = 5000,by = 500),las=2,labels = format(seq(from = 0,to = 5000,by = 500), big.mark = ".", decimal.mark = ","))

# SCATTERPLOT
corr<-cor.test(coredata(tsSSTINtotalNA),coredata(tsCODINtotalNA),method = "spearman", use="complete.obs",exact = F)

windows(width = 16,height = 9)
par(mar=c(6,6,4,4),mgp=c(4,1,0))
plot(as.zoo(tsCODINtotal),as.zoo(tsSSTINtotal),xaxs="i",yaxt="n",xaxt="n",yaxs="i",xlab=expression(paste("COD"[IN]," [mg/L]")),ylab=expression(paste("SST"[IN]," [mg/L]")),cex.lab="1.2",xlim=c(0,4000),ylim=c(0,5000),pch=16)
...
text(x=500,y=4500, label=paste("r = ",format(round(corr$estimate,digits=2),decimal.mark=",")))
