...
MAtsCaricoSSTINtotal<-na.omit(rollapply(xts(x=Caprino[,13], order.by=datestotal)*xts(x=Caprino[,6],order.by=datestotal)/1000, width=31, FUN=function(x) mean(x, na.rm=TRUE), by=1, by.column=TRUE, fill=NA, align="center"))
...
text(x=index(tsSSTIN2015[182,]),y=3300,label="2015")
text(x=index(tsSSTIN2016[182,]),y=3300,label="2016")
text(x=index(tsSSTIN2017[182,]),y=3300,label="2017")
text(x=index(tsSSTIN2018[90,]),y=3300,label="2018")
legend(x=index(tsSSTIN2017[90,]),y=3150, c(expression(paste("SST"[IN])),expression(paste("MA SST"[IN]," (mensile)"))),col=c("grey","black"),lty=c(1,1),lwd=c(1,1),bg="white")
