...
plot(as.zoo(tsNO3INtotal),type="n", xlab="Mesi",ylab=expression(paste("N-NO"[3-IN]^"-",", N-NO"[2-IN]^"-"," [mg/L]")),yaxt="n",xaxt="n",yaxs="i",xaxs="i",cex.lab=1.2,ylim=c(0,2.5),lwd=1,col="grey")
...
legend(x=index(tsNtotIN2015[40,]),y=2.3, c(expression(paste("N-NO"[3-IN]^"-")),expression(paste("N-NO"[2-IN]^"-")),expression(paste("MA N-NO"[3-IN]^"-", " (mensile)")),expression(paste("MA N-NO"[2-IN]^"+"," (mensile)"))),col=c("darkslategrey","orange","black","red"),lty=c(1,1,1,1),lwd=c(1,1,1,1),bg="white")

# BOXPLOT
windows(width = 6,height = 6)
par(mar=c(1,6,4,4),mgp=c(4,1,0))

boxplot(coredata(tsNO2INtotal),yaxt="n",ylab="[mg/L]",main=expression(paste("N-NO"[2-IN]^"-")), ylim=c(0,0.9),cex.lab=1.5,cex.main=2)
axis(side=2,at=seq(from = 0,to = 0.9,by = 0.1),las=2,labels = format(seq(from = 0,to = 0.9,by = 0.1), big.mark = ".", decimal.mark = ","))

windows(width = 6,height = 6)
par(mar=c(1,6,4,4),mgp=c(4,1,0))

boxplot(coredata(tsNO3INtotal),yaxt="n",ylab="[mg/L]",main=expression(paste("N-NO"[3-IN]^"-")), ylim=c(0,2.5),cex.lab=1.5,cex.main=2)
axis(side=2,at=seq(from = 0,to = 2.5,by = 0.5),las=2,labels = format(seq(from = 0,to = 2.5,by = 0.5), big.mark = ".", decimal.mark = ","))
