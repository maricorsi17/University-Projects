...
plot(as.zoo(tsCODINtotal),type="n", xlab="Mesi",ylab=expression(paste("COD"[OUT]," [mg/L]")),yaxt="n",xaxt="n",yaxs="i",xaxs="i",cex.lab=1.2,ylim=c(0,100),col="grey")
drawTimeAxis(as.zoo(tsCODINtotal_original), tick.tstep ="months", lab.tstep ="months",las=2,lab.fmt="%m")
axis(side=2,at=seq(from = 0,to = 100,by = 25),las=2,format(seq(from = 0,to = 100,by = 25), big.mark = ".", decimal.mark = ","))
...
text(x=index(tsCODIN2015[182,]),y=90,label="2015")
text(x=index(tsCODIN2016[182,]),y=90,label="2016")
text(x=index(tsCODIN2017[182,]),y=90,label="2017")
text(x=index(tsCODIN2018[90,]),y=90,label="2018")
text(x=index(tsCODIN2016[181,]),y=5, label=paste("Variazione = ",format(round(perc_a,digits=1),decimal.mark = ","),"%"),col="red")
legend(x=index(tsCODIN2017[60,]),y=85, c(expression(paste("COD"[OUT])),"Regressione","LOESS"),col=c("darkslategrey","red","black"),lty=c(1,5,1),lwd=c(1,2,2),bg="white")
