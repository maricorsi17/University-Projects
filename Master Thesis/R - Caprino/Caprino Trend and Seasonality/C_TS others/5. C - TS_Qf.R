...
MAtsQftotal<-na.omit(rollapply(xts(x=Caprino[,8], order.by=datestotal), width=31, FUN=function(x) mean(x, na.rm=TRUE), by=1, by.column=TRUE, fill=NA, align="center"))
...
text(x=index(tsQIN2015[181,]),y=225, label=paste("Variazione = ",format(round(perc_a,digits=1),decimal.mark = ","),"%"),col="red")
legend(x=index(tsQIN2017[80,]),y=270, c(expression("Q"[f]),"Regressione","LOESS"),col=c("darkslategrey","red", "black"),lty=c(1,5,1),lwd=c(1,2,2),bg="white")
