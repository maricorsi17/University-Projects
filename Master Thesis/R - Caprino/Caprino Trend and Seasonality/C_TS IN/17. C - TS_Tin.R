...
plot(as.zoo(tsTINtotal),type="n", xlab="Mesi",ylab=expression(paste("T"[IN]," [",degree,"C]")),yaxt="n",xaxt="n",yaxs="i",xaxs="i",cex.lab=1.2,ylim=c(5,30))
...
lines(as.zoo(tsTINtotal),col="darkslategrey")
lines(as.zoo(ts_trend(tsTINtotal)),lwd=2)
a<-lm(tsTINtotal~index(tsTINtotal))
...
text(x=index(tsTIN2015[182,]),y=29,label="2015")
text(x=index(tsTIN2016[182,]),y=29,label="2016")
text(x=index(tsTIN2017[182,]),y=29,label="2017")
text(x=index(tsTIN2018[90,]),y=29,label="2018")
text(x=index(tsTIN2016[181,]),y=7, label=paste("Variazione = ",format(round(perc_a,digits=1),decimal.mark = ","),"%"),col="red")
legend(x=index(tsTIN2017[340,]),y=27.5, c(expression(paste("T"[IN])),"Regressione","LOESS"),col=c("darkslategrey","red","black"),lty=c(1,5,1),lwd=c(1,2,2),bg="white")

# DETRENDING
tsTINtotalDET<-tsTINtotal-(ts_trend(tsTINtotal))
mediaDET<-mean(tsTINtotalDET)

# STAGIONALITA'
tsTINtotalAGG<-apply.weekly(tsTINtotalDET,median)

windows(width = 16,height = 9)
par(mar=c(6,6,4,4),mgp=c(3,1,0),cex.main=2)
options(OutDec= ",")
acf(tsTINtotalAGG,lag.max = 60, main=expression("T"[IN]),yaxt="n", ci.col="black",cex.lab=1.2)
axis(side=2,las=2)
