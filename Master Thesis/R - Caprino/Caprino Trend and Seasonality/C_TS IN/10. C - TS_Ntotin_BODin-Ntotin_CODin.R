...
text(x=index(tsBODIN2016[181,]),y=0.45, label=paste("Variazione = ",format(round(perc_a,digits=1),decimal.mark = ","),"%"),col="red")
legend(x=index(tsBODIN2015[30,]),y=0.62, c(expression(paste("N"[tot-IN],"/BOD"[5-IN])),"Regressione","LOESS"), col=c("darkslategrey","red","black"),lty=c(1,5,1),lwd=c(1,2,2),bg="white")
...
text(x=index(tsBODIN2016[181,]),y=0.18, label=paste("Variazione = ",format(round(perc_a1,digits=1),decimal.mark = ","),"%"),col="red")
legend(x=index(tsBODIN2015[30,]),y=0.27, c(expression(paste("N"[tot-IN],"/COD"[IN])),"Regressione","LOESS"), col=c("darkslategrey","red","black"),lty=c(1,5,1),lwd=c(1,2,2),bg="white")


# DETRENDING
rapportoN_BODDET<-rapportoN_BOD-(ts_trend(rapportoN_BOD))
mediarapp_NBODDET<-mean(rapportoN_BODDET)
