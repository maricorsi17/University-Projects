...
text(x=index(tsPtotIN2016[181,]),y=0.005, label=paste("Variazione = ",format(round(perc_a,digits=1),decimal.mark = ","),"%"),col="red")
...
text(x=index(tsPtotIN2016[181,]),y=0.022, label=paste("Variazione = ",format(round(perc_a1,digits=1),decimal.mark = ","),"%"),col="red")
legend(x=index(tsPtotIN2015[30,]),y=0.035, c(expression(paste("P"[tot-IN],"/COD"[IN])),"Regressione","LOESS"),col=c("darkslategrey","red","black"),lty=c(1,5,1),lwd=c(1,2,2),bg="white")
